...
export async function seedIfEmpty() {
  const { rows } = await pool.query("SELECT COUNT(*)::int AS count FROM competitors");
  if (rows[0].count > 0) return; // admin data already exists, never overwrite

  console.log("First run: seeding initial competitor data...");
  ...
  try {
    ...
    console.log("Seed complete: 4 competitors, 24 CNPJs inserted.");
  } catch (e) {
